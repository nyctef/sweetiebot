...
class SweetieTell(object):
    ...
    def tell(self, message):
        ...
        if sendee_nick and mess:
            self.storage.set_or_update_message(sendee_jid, sender_jid, '{} left you a message: {}'.format(sender_nick, mess))
            return 'Message received for {}'.format(sendee_jid)

    def get_messages_for(self, message):
        messages = self.storage.get_existing_messages_by_sender(message.user_jid).values()
        if len(messages):
