...
class PingTests(unittest.TestCase):
    ...
    def test_basic_ping_workflow(self):
        ...
        self.assertEqual(
            "Available groups: " + ". See also !users and !mygroups", response
        )

        # one user subscribes to a group
        response = self.pings.subscribe(
            create_message_zhuli("!subscribe capable_assistants")
        )
        self.assertEqual(
            "User zhuli@jabber.org added to group 'capable_assistants'", response
        )

        # the list of groups can now be queried again
        response = self.pings.groups(create_message("!groups"))
        self.assertEqual(
            "Available groups: capable_assistants (1). See also !users and !mygroups",
            response,
        )
        ...
        self.assertEqual(
            "User sender@jabber.org is not currently subscribed to any pingable groups",
            response,
        )

        # A user now pings the group to send a message
        response = self.pings.ping(
            create_message("!ping capable_assistants Do the thing!")
        )
        ...
        response = self.pings.unsubscribe(
            create_message_zhuli("!unsub capable_assistants")
        )
        self.assertEqual(
            "User zhuli@jabber.org removed from group 'capable_assistants'", response
        )

        response = self.pings.mygroups(create_message_zhuli("!mygroups"))
        self.assertEqual(
            "User zhuli@jabber.org is not currently subscribed to any pingable groups",
            response,
        )
